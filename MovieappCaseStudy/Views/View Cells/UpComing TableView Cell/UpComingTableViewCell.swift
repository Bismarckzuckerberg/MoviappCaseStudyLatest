...
class UpComingTableViewCell: UITableViewCell {
    ...
    func setData() {
        ...
        if let imageUrl = self.dataSource?.backdropPath {
            self.posterImage.sd_setImage(with: URL(string: (Domain.tmdbImageUrl + imageUrl)), placeholderImage: #imageLiteral(resourceName: "emptyimage"), options: SDWebImageOptions.scaleDownLargeImages, context: nil)
        }
